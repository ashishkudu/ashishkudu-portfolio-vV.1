...
## ASH data storage (Version 1.1)

This version stores ASH conversations in Vercel Postgres/Neon when `POSTGRES_URL` is configured. The API creates the `ash_conversations` table automatically on the first stored conversation.

### Required production environment variables

- `GROQ_API_KEY` — Groq API key used by ASH.
- `POSTGRES_URL` — Vercel Postgres/Neon connection string.
- `GROQ_MODEL` — optional; defaults to `llama-3.1-8b-instant`.

The visitor receives an anonymous browser session ID. Stored records include the session ID, user question, ASH answer, user-agent, referrer, and timestamp. Visitors can delete records for their current session from the ASH panel.

See `/privacy` for the public data-collection policy.
