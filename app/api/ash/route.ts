...
import { deleteAshConversation, storeAshConversation } from "@/lib/db";
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({
        answer:
          "ASH is not connected yet. Add GROQ_API_KEY to your deployment environment to activate the Groq AI connection."
      });
    }
    ...
    if (!response.ok) {
      ...
      const detail = data?.error?.message || `Groq returned HTTP ${response.status}`;
      return NextResponse.json(
    ...
    if (sessionId) {
      try {
        stored = await storeAshConversation({
          sessionId,
          userMessage: message,
        ...
        console.error("ASH storage error:", storageError);
      }
    }

    return NextResponse.json({ answer, stored });
  } catch (error: unknown) {
...
export async function DELETE(request: Request) {
  try {
    ...
    await deleteAshConversation(sessionId);
    return NextResponse.json({ deleted: true });
  } catch (error: unknown) {
    console.error("ASH deletion error:", error);
